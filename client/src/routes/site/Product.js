        ...
        <Grid container justify="center" spacing={24}>
          ...
          <Grid item xs={12} md={7} lg={8}>
            <Breadcrumb
              match={match}
              links={[
                { name: product.type.category.name, type: 'catSlug' },
                { name: product.type.name, type: 'typeSlug' },
                { name: product.name, type: 'productSlug' },
              ]}
            />
          </Grid>
        </Grid>
...
const productWithData = graphql(productQuery, {
  options: props => ({ variables: { value: props.match.params.productSlug } }),
})(Product);

export default compose(withStyles(style), withWidth())(productWithData);
